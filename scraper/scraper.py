...
with open("config.json") as fp:
    ...
    DISPLAY_ERRORS = config.get("displayErrors", True)
    MIN_MEMBER_COUNT = config.get("minMemberCount")
    IS_LOOPED = config.get("isLooped", False)
    WEBHOOK_URL = config.get("webhookUrl")
    RANGE = config["range"] if len(sys.argv) == 1 else {"min": int(sys.argv[1]), "max": int(sys.argv[2])}
...
id_iter = iter(range(RANGE["min"], RANGE["max"] + 1))
